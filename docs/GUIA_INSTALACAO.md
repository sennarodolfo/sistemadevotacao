...
22. **SQL Editor → New query**: cole o conteúdo de `supabase/migrations/0019_auditoria_por_sessao.sql` e rode (Auditoria passa a listar comprovantes por sessão).
23. **SQL Editor → New query**: cole o conteúdo de `supabase/seed/seed.sql` e rode.
24. Copie o UUID da eleição mostrado na mensagem final: este é seu `VITE_ELECTION_ID` — **só necessário se for usar o modo clássico de uma eleição só**. No modo multiusuário (recomendado), pule este passo.
25. **Settings → API** copie:
