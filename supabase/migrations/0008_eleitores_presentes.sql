-- =================================================================
-- Migração 0008: Número de eleitores presentes por sessão
-- =================================================================
-- Eleição por assembleia: cada sessão tem um número FIXO de eleitores
-- presentes (informado pelo admin), e o candidato é eleito com "50% dos
-- votos mais um" EM CIMA DESSE NÚMERO - não em cima do total de votos
-- válidos. Esta migração:
--
--   1) Adiciona voting_sessions.registered_voters (número de eleitores
--      presentes na assembleia para aquela sessão).
--   2) Passa esse número a admin_create_session / admin_update_session,
--      get_public_election, admin_get_results e ao backup
--      (admin_export_election / admin_import_election).
--
-- O CÁLCULO do percentual (votos do candidato / eleitores presentes)
-- é feito no frontend (ResultsView / PublicResultsScreen), que já
-- recebe esse número via admin_get_results - nenhuma trava é aplicada
-- no banco, o campo é apenas informativo/base de cálculo.
-- =================================================================

alter table public.voting_sessions add column if not exists registered_voters integer;

-- ============== admin_create_session (+ registered_voters) ==============
...
begin
  ...
  insert into public.voting_sessions (election_id, title, votes_required, registered_voters, display_order)
  values (p_election_id, p_title, p_votes_required, p_registered_voters, new_order)
...
grant execute on function public.admin_create_session(uuid, text, text, integer, integer, jsonb) to anon, authenticated;

-- ============== admin_update_session (+ registered_voters) ==============
...
grant execute on function public.admin_update_session(uuid, text, uuid, text, integer, integer, jsonb, boolean) to anon, authenticated;

-- ============== get_public_election: inclui registered_voters ==============
...
-- ============== admin_get_results: inclui registered_voters ==============
...
begin
  ...
  from (
    ...
    group by vs.id, vs.title, vs.votes_required, vs.registered_voters, vs.display_order
    order by vs.display_order
  ) r;
...
-- ============== admin_export_election: inclui registered_voters ==============
...
-- ============== admin_import_election: restaura registered_voters ==============
